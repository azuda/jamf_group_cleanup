...
SMART_GROUP_XML = """<computer_group>
  <id>10</id><name>Smart Group A</name><is_smart>true</is_smart>
  <criteria>
    <criterion>
      <name>Computer Group</name>
      <value>Old Group</value>
    </criterion>
  </criteria>
</computer_group>"""

SMART_GROUP_XML_AFTER = """<computer_group>
  <id>10</id><name>Smart Group A</name><is_smart>true</is_smart>
  <criteria>
    <criterion>
      <name>Computer Group</name>
...
def _make_rs_with_sg(smart_groups, objects=None):
  return ResolvedScope(
    source_id=1, source_name="Old Group",
    target_id=2, target_name="New Group",
    group_type="computer",
...
def _make_sg(already=False):
  return SmartGroupCriterionRef(
    group_id=10, group_name="Smart Group A", target_already_present=already,
  )
...
# ── execute_scope — smart group criterion update ─────────────────────────────

def test_execute_scope_updates_smart_group_criterion():
  rs = _make_rs_with_sg([_make_sg()])
  with patch("scope_executor.classic_get", return_value=_mock_response(200, SMART_GROUP_XML)), \
       patch("scope_executor.put_with_retry", return_value=_mock_response(201)):
    results = scope_executor.execute_scope([rs], _make_token(), MagicMock())
  assert results[0].status == "OK"
...
def test_execute_scope_skips_sg_already_updated_by_another_admin():
  rs = _make_rs_with_sg([_make_sg()])
  # fresh GET shows source criterion already gone
  with patch("scope_executor.classic_get", return_value=_mock_response(200, SMART_GROUP_XML_AFTER)), \
       patch("scope_executor.put_with_retry") as mock_put:
    results = scope_executor.execute_scope([rs], _make_token(), MagicMock())
  mock_put.assert_not_called()
  assert results[0].status == "OK"
  assert results[0].smart_groups_updated == []


def test_execute_scope_sg_put_error_but_change_applied():
  rs = _make_rs_with_sg([_make_sg()])
  get_responses = [
    _mock_response(200, SMART_GROUP_XML),       # initial GET — source criterion present
    _mock_response(200, SMART_GROUP_XML_AFTER), # verify GET — source criterion gone
  ]
  with patch("scope_executor.classic_get", side_effect=get_responses), \
       patch("scope_executor.put_with_retry", return_value=_mock_response(409, "Problem")):
    results = scope_executor.execute_scope([rs], _make_token(), MagicMock())
  assert results[0].status == "OK"
  assert len(results[0].smart_groups_updated) == 1


def test_execute_scope_sg_true_fail():
  rs = _make_rs_with_sg([_make_sg()])
  get_responses = [
    _mock_response(200, SMART_GROUP_XML),  # initial GET
    _mock_response(200, SMART_GROUP_XML),  # verify GET — source still present
  ]
  with patch("scope_executor.classic_get", side_effect=get_responses), \
       patch("scope_executor.put_with_retry", return_value=_mock_response(500, "err")):
    results = scope_executor.execute_scope([rs], _make_token(), MagicMock())
  assert results[0].status == "FAIL"
